...
import RecablesTrans from "./RecablesTrans";
...
import PrintRecables from "../../../Prints/AcPrints/PrintRecables";

export default function Recables() {
  //#region variables
  ...
  function GenerateReport() {
    ...
    var config = {
      method: "GET",
      url: `api/fin/recables?sdate=${sDateRef.current.value}&edate=${eDateRef.current.value}${cri}&orderby=partytypeid,partyname`,
    };
    axios(config)
      .then((response) => {
        setIsLoading(false);
    ...
    >
      <div className="main-div">
        ...
        <div className="printing_area" ref={componentRef}>
          <br />
          <RecablesTrans transarray={ftransarray} />
        </div>
      </div>
      <div>
        <PrintRecables
          parentobj={{
            SDate: sDateRef?.current?.value,
            EDate: eDateRef?.current?.value,
          }}
