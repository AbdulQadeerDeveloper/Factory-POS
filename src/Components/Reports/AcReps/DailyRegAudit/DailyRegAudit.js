...
import DailyRegAuditTran from "./DailyRegAuditTran";
...
export default function DailyRegAudit() {
  //#region variables
  ...
  function GenerateReport() {
    ...
    var config = {
      method: "GET",
      url: `api/fin/dailyregaudit?sdate=${sDateRef.current.value}&edate=${eDateRef.current.value}`,
    };
    // 131010060
    axios(config)
      .then((response) => {
    ...
    <AnimatedPage className="container-fluid main_container">
      <div className="main-div">
        ...
        <div className="printing_area" ref={componentRef}>
          <br />
          <DailyRegAuditTran transarray={ftransarray} setTransarray={setTransarray} />
        </div>
      </div>
    </AnimatedPage>
  );
