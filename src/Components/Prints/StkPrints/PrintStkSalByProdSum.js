...
export default function PrintStkSalByProdSum({
  parentobj,
  componentRef,
  transarray,
  isSum = false
  ...
  function G1FooterRow({ index }) {
    if (isLastG1(index)) {
      return (
        <tr className="right bold" style={{ borderBottom: "1px solid #000" }}>
          <td colSpan={2} >
            <span>{transarray[index].ProdName} Total:</span>
          </td>
          <td className="center">
            <Currency value={g1_pbal} />
          </td>
      ...
      return (
        <tr className={`right ${isSum ? 'normal' :  'bold'}`}>
          <td colSpan={2} style={{ fontSize: "8px"}}>
            {transarray[index].PartyName.substring(0, 30)} Total:
          </td>
          <td className="center">
  ...
  function isNewG2(index) {
    return index === 0 || transarray[index].PartyName !== transarray[index - 1].PartyName || transarray[index].ProdName !== transarray[index - 1].ProdName
  }
  ...
  function isLastG2(index) {
    const lastIndex = transarray.length - 1;
    return index === lastIndex || transarray[index].PartyName !== transarray[index + 1].PartyName || transarray[index].ProdName !== transarray[index + 1].ProdName
  }
