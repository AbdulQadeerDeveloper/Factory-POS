...
import PrintSRV from "../../../Prints/StkPrints/PrintSRV";
import { GlobalData } from "../../../GlobalData";

export default function SRVTrans(props) {
  ...
  function FocusNextInputChild(e) {
    ...
    if (PQtyRef.current === document.activeElement) {
      AvgRateRef.current && AvgRateRef.current.focus();
    } else if (PackingRef.current === document.activeElement) {
      AvgRateRef.current && AvgRateRef.current.focus();
    } else if (AvgRateRef.current === document.activeElement) {
      ...
      if (localStorage.getItem("upd-sr")) {
        UpdatedSingle();
      } else {
        AddSingle();
      }
  ...
  function UpdSingle(index) {
    ...
    localStorage.setItem("upd-sr", JSON.stringify(localarr));
    setMsg({ ...msg, err: "", color: "" });
    AdvRef.current && AdvRef.current.focus();
  }
  ...
  function UpdatedSingle() {
    if (HandleValidate()) {
      let data = JSON.parse(localStorage.getItem("upd-sr"));
      const val = [...transarray];
  ...
  function AvgRateFun(e) {
    let avgRate = parseFloat(e.target.value);
    if (isNaN(avgRate)) {
      setChildobj({ ...childobj, AvgRate: null });
    } else {
      setChildobj({ ...childobj, AvgRate: avgRate });
    }
  }
  function RateFun(e) {
            ...
            <tr>
              ...
              <td>
                <input
                  ...
                  className="form-control POOthers"
                  autoComplete="off"
                  onChange={(e) => AvgRateFun(e)}
                  value={childobj.AvgRate !== null ? childobj.AvgRate : ""}
                />
              </td>
              <td>
                <input
