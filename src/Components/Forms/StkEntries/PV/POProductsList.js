...
function POProductsList(props) {
    const contextdata = useContext(GlobalData);

    const { childobj, setChildobj, PQtyRef, ProductRef, parentobj, NextElemRef } = props;
    ...
    useEffect(() => {
        ...
        if (parentobj.PartyId !== null && parentobj.FirmId !== null) {
            var config = {
                method: 'GET',
                url: `api/po/po_bal?cri=partyid=${parentobj.PartyId} AND firmid=${parentobj.FirmId}`
            };
            axios(config)
                .then((response) => {
