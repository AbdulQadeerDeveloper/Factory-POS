...
export default function POTrans(props) {
  const {
    ChildObjEmpty,
    childobj,
    setChildobj,
  ...
  function FocusNextInputChild(e) {
    ...
    if (PQtyRef.current === document.activeElement) {
      ...
      if (localStorage.getItem("upd-po")) {
        UpdatedSingle();
      } else {
        AddSingle();
      }
  ...
  function UpdSingle(index) {
    ...
    localStorage.setItem("upd-po", JSON.stringify(localarr));
    setMsg({ ...msg, err: "", color: "" });
    AdvRef.current && AdvRef.current.focus();
  }
  ...
  function UpdatedSingle() {
    if (HandleValidate()) {
      let data = JSON.parse(localStorage.getItem("upd-po"));
      const val = [...transarray];
