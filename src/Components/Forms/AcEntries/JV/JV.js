...
export default function JV() {
  ...
  const ParentObjEmpty = useCallback(() => {
  ...
  const getData = useCallback(
    ...
    [ParentObjEmpty, msg, navigate, parentobj]
  );

  const IsDrCrEqual = () => {
    const b = transarray.reduce((acc, curr) => {
      return (acc + curr.NetDebit - curr.NetCredit)
    }, 0)
    return (b===0)
  ...
  function handleKeyDown(event) {
    if (event.ctrlKey && event.key === "Enter") {
      if (parentobj.VocNo===0 && IsDrCrEqual())
        SubmitVoucher()
    }
  }
      ...
      <div className="container-fluid main_container">
        <div className="main-div">
          ...
          <div className="page_caption_area">
            ...
            {/* <div className={JVStyle.spacer}></div> */}
